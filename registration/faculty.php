<?php
// Enable error reporting for debugging
error_reporting(E_ALL);
ini_set('display_errors', 1);

// Include the database connection file
include '../db_conn.php'; // Adjust the path based on your directory structure

// Initialize messages
$success_message = '';
$error_message = '';

// Check if the form is submitted
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $faculty = trim($_POST['faculty']);

    // Basic validation
    if (empty($faculty)) {
        $error_message = "Faculty name cannot be empty.";
    } elseif (strlen($faculty) > 100) {
        $error_message = "Faculty name is too long.";
    } else {
        // Prepare and bind SQL statement
        $stmt = $conn->prepare("INSERT INTO faculty (faculty) VALUES (?)");
        $stmt->bind_param("s", $faculty);

        // Execute the statement
        if ($stmt->execute()) {
            $success_message = "Faculty added successfully!";
            // Redirect to the same page to prevent form resubmission
            header('Location: faculty.php');
            exit(); // Ensure no further code is executed after redirect
        } else {
            $error_message = "Error: " . $stmt->error;
        }

        // Close the statement
        $stmt->close();
    }

    // Close the connection
    $conn->close();
}
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Add Faculty</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 py-10">

    <div class="max-w-md mx-auto bg-white p-8 rounded-lg shadow-lg">
        <h2 class="text-2xl font-bold text-center mb-6">Add Faculty</h2>
        
        <!-- Success/Error Messages -->
        <?php if ($success_message): ?>
            <div class="text-green-600 mb-4 text-center"><?php echo htmlspecialchars($success_message); ?></div>
        <?php endif; ?>
        <?php if ($error_message): ?>
            <div class="text-red-600 mb-4 text-center"><?php echo htmlspecialchars($error_message); ?></div>
        <?php endif; ?>

        <form action="" method="post" class="space-y-4">
            <div>
                <label for="faculty" class="block text-lg font-semibold mb-2">Faculty Name</label>
                <input type="text" id="faculty" name="faculty" placeholder="Enter faculty name" required 
                    class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
            </div>
            <div>
                <input type="submit" value="Add Faculty" 
                    class="w-full bg-green-500 text-white py-3 rounded-lg hover:bg-green-600 transition duration-300 cursor-pointer">
            </div>
        </form>
    </div>

</body>
</html>
