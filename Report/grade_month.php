...
if ($stmt->execute()) {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $employees[$row['grade']][$row['designation']]['details'][] = $row;
            $employees[$row['grade']][$row['designation']]['count'] = 
                isset($employees[$row['grade']][$row['designation']]['count']) 
                    ? $employees[$row['grade']][$row['designation']]['count'] + 1 
                    : 1;
        }
    } else {
...
<body>
    <main class="flex flex-col flex-grow bg-white px-8 pb-20 mb-20">
        ...
        <?php ksort($employees); foreach ($employees as $gradee => $designations): ?>
