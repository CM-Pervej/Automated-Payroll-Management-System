...
// Initialize an empty array for storing payroll data per grade
$gradeData = [];
...
while (($current_year < $end_year) || ($current_year == $end_year && $current_month <= $end_month)) {
    // Prepare SQL query to get data per grade
    $stmt = $conn->prepare("SELECT * FROM payroll WHERE `month` = ? AND `year` = ? GROUP BY grade, employee_id");
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            // Group payroll data by grade
            $grade = $row['grade'];

            // Initialize grade if not already done
            ...
            // Update grade totals
            $gradeTotals[$grade]['totalEmployees']++;
            $gradeTotals[$grade]['totalBasic'] += $row['basic'];
            $gradeTotals[$grade]['totalAllowances'] += $totalAllowance;
            $gradeTotals[$grade]['totalDeductions'] += $totalDeduction;
            $gradeTotals[$grade]['totalSalary'] += $netPay;
        }
    }

    // Store the grade totals for the current month
    foreach ($gradeTotals as $grade => $totals) {
...
<head>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            // Loop through each grade and apply the number to words function
            document.querySelectorAll('.net-pay-words').forEach(function(element) {
                const gradeSum = element.getAttribute('data-total'); // Get the total salary from a custom data attribute
                const sum = parseFloat(gradeSum.replace(/,/g, '')); // Convert to a float and check for commas
                console.log('Sum for grade: ', sum); // Check the sum in the console
...
<body class="bg-gray-100 px-8 text-black">
    <?php foreach ($gradeData as $grade => $data): ?>
