...
// Initialize an empty array for storing payroll data per department
$departmentData = [];
...
while (($current_year < $end_year) || ($current_year == $end_year && $current_month <= $end_month)) {
    // Prepare SQL query to get data per department
    $stmt = $conn->prepare("SELECT * FROM payroll WHERE `month` = ? AND `year` = ? GROUP BY departments, employee_id");
    $stmt->bind_param("ii", $current_month, $current_year);
    $stmt->execute();
    $result = $stmt->get_result();

    // Initialize totals for each department
    $departmentTotals = [];
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            // Initialize department if not already done
            ...
            $departmentTotals[$department]['totalEmployees']++;
            $departmentTotals[$department]['totalBasic'] += $row['basic'];
            $departmentTotals[$department]['totalAllowances'] += $totalAllowance;
            $departmentTotals[$department]['totalDeductions'] += $totalDeduction;
            $departmentTotals[$department]['totalSalary'] += $netPay;
        }
    }

    // Store the department totals for the current month
...
<head>
    ...
    <title>Payroll Report by Department</title>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelectorAll('.net-pay-words').forEach(function(element) {
                const departmentSum = element.getAttribute('data-total'); // Get the total salary from a custom data attribute
                const sum = parseFloat(departmentSum.replace(/,/g, '')); // Convert to a float and check for commas
                console.log('Sum for department: ', sum); // Check the sum in the console
...
<body class="bg-gray-100 px-8 text-black">
    <?php foreach ($departmentData as $department => $data): ?>
    <section class="page-container pt-5 relative h-screen flex flex-col gap-10">
        <section class="department-section">
            ...
            <p class="text-gray-600 text-lg font-bold">Department: <span class="font-normal"><?php echo $department; ?></span></p>
