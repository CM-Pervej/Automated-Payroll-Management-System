...
if ($stmt->execute()) {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $employees[$row['designation']][$row['departments']]['details'][] = $row;
            $employees[$row['designation']][$row['departments']]['count'] = 
                isset($employees[$row['designation']][$row['departments']]['count']) 
                    ? $employees[$row['designation']][$row['departments']]['count'] + 1 
                    : 1;
        }
    } else {
...
<body>
    <main class="flex flex-col flex-grow bg-white px-8 pb-20 mb-20">
        ...
        <?php ksort($employees); foreach ($employees as $designation => $departments): ?>
    <section class="page-container pt-16">
        ...
        <section class="mt-5">
            <p class="text-xl">Salary Details of <span class="font-bold"><?php echo htmlspecialchars($designation); ?></span> </p>
            <article class="mt-5">
                <table class="w-full border-collapse border border-black">
                    ...
                    <tbody>
                            <?php 
                            $totalDesignationPay = 0; 
                            $sl = 1;
                            foreach ($departments as $department => $data): 
                                $totalDepartmentPay = array_sum(array_column($data['details'], 'netPay'));
                                $totalDesignationPay += $totalDepartmentPay;
                            ?>
                                <tr class="hover:bg-gray-100 text-black">
                                    ...
                                    <td class="border border-gray-300 px-4 py-2 text-left whitespace-nowrap"><?php echo htmlspecialchars($department); ?></td>
