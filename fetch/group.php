...
$query = "SELECT e.*, d.department_name, des.designation 
          FROM employee e 
...
$query .= " ORDER BY e.grade_id ASC, d.department_name, e.name";
                        ...
                        <section class="mb-8">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                <?php foreach ($employees as $employee): ?>
                                    <div class="bg-blue-50 p-6 rounded-lg shadow-lg hover:shadow-2xl transition-shadow border">
                                        <div class="flex items-center justify-center mb-4">
                                            <?php 
                                                if (!empty($employee['image'])) {
                                                    $imagePath = '../uploads/' . basename($employee['image']);
                                                    if (file_exists($imagePath)) {
                                                        echo '<img src="' . $imagePath . '" alt="Profile Image" class="w-24 h-24 object-cover rounded-full border-2 border-gray-300" />';
                                                    } else {
                                                        echo '<img src="uploads/default-avatar.png" alt="Profile Image" class="w-24 h-24 object-cover rounded-full border-2 border-gray-300" />';
                                                    }
                                                } else {
                                                    echo '<img src="uploads/default-avatar.png" alt="Profile Image" class="w-24 h-24 object-cover rounded-full border-2 border-gray-300" />';
                                                }
                                        ...
                                        <h2 class="text-xl font-semibold mb-2">
                                            <a href="/payroll/profile.php?employee_id=<?php echo $employee['id']; ?>" class="text-blue-600 hover:underline">
                                                <?= htmlspecialchars($employee['name']); ?>
                                            </a>
                                        </h2>

                                        <p class="text-sm text-gray-600">Employee No: <?= htmlspecialchars($employee['employeeNo']); ?></p>
                                        <p class="text-sm text-gray-600">Contact: <?= htmlspecialchars($employee['contactNo']); ?></p>
                                        <p class="text-sm text-gray-600">Email: <?= htmlspecialchars($employee['email']); ?></p>
                                        <p class="text-sm text-gray-600">Designation: <?= htmlspecialchars($employee['designation']); ?></p>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        </section>
