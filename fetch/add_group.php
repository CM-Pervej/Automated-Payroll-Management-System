...
$query = "SELECT e.*, d.department_name, des.designation 
          ...
          WHERE e.empStatus = ? AND approve = 1"; // Filter by empStatus (1 for active, 2 for inactive)
        ...
        <main class="flex-grow p-8 mt-5 bg-white shadow-lg overflow-auto">
            <div class="mx-auto bg-white p-10">
                ...
                <div class="flex justify-end">
                    ...
                    <a href="?empStatus=1&grade_id=<?= $grade_id ?>&designation_id=<?= $designation_id ?>&department_id=<?= $department_id ?>" 
                    class="<?= $empStatus == 1 ? 'hidden' : 'text-blue-600 text-xl font-bold hover:underline' ?>">Inctive Employees</a>
                    
                    <!-- Inactive Employees Button -->
                    <a href="?empStatus=2&grade_id=<?= $grade_id ?>&designation_id=<?= $designation_id ?>&department_id=<?= $department_id ?>" 
                    class="<?= $empStatus == 2 ? 'hidden' : 'text-blue-600 text-xl font-bold hover:underline' ?>">Active Employees</a>
                </div>
                                ...
                                <?php foreach ($employees as $employee): ?>
                                    <a href="/payroll/profile.php?employee_id=<?php echo $employee['id']; ?>" class="block bg-blue-50 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 ease-in-out hover:scale-105 hover:bg-blue-100 hover:border-2 hover:border-blue-300">
                                        ...
                                        <div class="px-6 pb-6">
                                            ...
                                            <p class="text-sm text-gray-600 my-1"><i class="fas fa-envelope mr-2"></i><?= htmlspecialchars($employee['email']); ?></p>
